...
namespace TDE
{
   // [Serializable]
    public class T_Line : T_Graphic
    {
        ...
        public T_Line(T_Line line) : base(line)
        {
            localEndPos = new Vector2ReactiveProperty(line.localEndPos.Value);
            localOriginPos = new Vector2ReactiveProperty(line.localOriginPos.Value);
            lineShapeType = new ReactiveProperty<LineShapeType>(line.lineShapeType.Value);
            lineBeginShapeType = new ReactiveProperty<LineBeginShape>(line.lineBeginShapeType.Value);
            lineEndShapeType = new ReactiveProperty<LineEndShape>(line.lineEndShapeType.Value);
            px = new IntReactiveProperty(line.px.Value);
        }
