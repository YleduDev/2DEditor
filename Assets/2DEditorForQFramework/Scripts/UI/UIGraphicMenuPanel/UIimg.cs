...
namespace QFramework.TDE
{
    public partial class UIimg : UIElement,IDragHandler,IBeginDragHandler,IEndDragHandler
	{
        ...
        internal void Init(string key,RectTransform Viewport,Action act)
        {
            this.spriteKey = key;
            ...
            UIimgDeleteBtn.onClick.AddListener(() =>  act?.Invoke());
        }

        public void SetUIimgBtnActive(bool bo)
        {
            if (bo) UIimgDeleteBtn.Show(); else UIimgDeleteBtn.Hide();
        }

        public void SettextrueData(string data)
        {
            // File.WriteAllText(FilePath.StreamingAssetsPath+"3.text", data,System.Text.Encoding.UTF8);
           // Log.I(data);
            if (!data.IsNullOrEmpty()) textrueData = data;
        ...
        public void OnDrag(PointerEventData eventData)
        {
            ...
            if (!FilterLocalPosForUIGraphicVw(localPoint, out bo))
            {
                ...
                if (bo&&T_Graphic.IsNotNull())
                {
                    ...
                    else if(T_Graphic.graphicType == GraphicType.Text) tParent = Global.textParent;
